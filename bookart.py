...
from inkex import (Boolean, Circle, ClipPath, EffectExtension, Ellipse, Group,
                   Layer, PathElement, Rectangle, TextElement, Transform,
                   Tspan)
...
class Design:
    ...
    def elements(self):
        ...
        if not self.svg.selection:
            ...
            pattern_elements = list(pattern_elements.get((PathElement, Ellipse, Rectangle, Circle)))
...
class Lines:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, design_colors, num_lines, num_pages,  # pylint: disable=too-many-arguments
                 design_bbox, design_clips, settings):
            ...
            for j in range(num_colors):
                ...
                for k in range(self.num_lines):
                    ...
                    if j == num_colors - 1 and line_number % 10 != 0:
                        ...
                        line_number += 1
                        continue
                    if j == num_colors - 2 and (line_number % 10 == 0 or line_number % 5 != 0):
                        left += self.settings['line_distance']
                        line_number += 1
                        continue
                    if j == num_colors - 3 and line_number % 5 == 0:
                        ...
                        line_number += 1
                        continue
                    path += f"M {left} {line_bbox['top']} L {left} {line_bbox['bottom']} "
                    left += self.settings['line_distance']
                    line_number += 1
                lines.set('d', path)
...
class Text:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, font_size, y_position):
        ...
        self.text_element = TextElement(y=str(y_position + self.font_size + 2), style=self.style)
...
class Pages:
    ...
    def num_pages(self):
        ...
        num_pages = self.settings['line_distance'] * \
            self.settings['total_pages'] / self.page_content_area()
    ...
    def document_bbox(self, document_format):
        ...
        if document_format == "letter":
            # US Letter: 8.5 * 11 (in)
            self.width, self.height = (self.svg.viewport_to_unit('8.5in'),
                                       self.svg.viewport_to_unit('11in'))
        else:
            # A4: 210 x 297 (mm)
            self.width, self.height = (self.svg.viewport_to_unit('210mm'),
...
class Bookart(EffectExtension):
    ...
    def add_arguments(self, pars):
        ...
        pars.add_argument("--color_highlight1", type=Color,
                          default=Color('#ed333b'))
        pars.add_argument("--color_highlight2", type=Color,
                          default=Color('#ffbe6f'))
        pars.add_argument("--color_background", type=Color,
    ...
    def effect(self):
        ...
        design_pages = total_pages - self.options.pages_before - self.options.pages_after
        ...
        self.settings = {
            ...
            'page_margins': self.convert_unit(self.options.page_margins, self.options.margin_unit),
